...
let list = Array.from(document.querySelectorAll('li .character')).map(toon => { return [
    ...
    Number(toon.querySelectorAll('p strong')[3].innerText.replace('None', '0'))
 	];
}, [])
	.sort(function(a, b) {
    return b[1] - a[1];
...
console.log("%Sorted Damage\t\t\t\t", consoleTitleLineStyling);
console.log('%c\tDamage\tName\t\t\t\t', consoleStyling);
